...
import {
  ...
  Undo2, Trash2, LayoutGrid, SlidersHorizontal, EyeOff, Eye, X, Check,
  Minimize2, RotateCcw, Save, Hand,
} from 'lucide-react'
import { brl } from '../lib/format'
import { ALAVANCAS, TINTAS } from '../lib/apresentacao'
...
const FERRAMENTAS = [
  { id: 'apontar', icone: Hand, rotulo: 'Passar slide', dica: 'A caneta volta a rolar a página e clicar nos botões' },
  { id: 'laser', icone: MousePointer2, rotulo: 'Apontar', dica: 'Rastro que apaga sozinho — para mostrar sem sujar' },
  { id: 'caneta', icone: PenLine, rotulo: 'Caneta', dica: 'Escreve e circula, com a pressão da Apple Pencil' },
  { id: 'marcador', icone: Highlighter, rotulo: 'Marcador', dica: 'Grifa por cima sem esconder o texto' },
  { id: 'borracha', icone: Eraser, rotulo: 'Borracha', dica: 'Encoste num traço e ele some inteiro' },
]

const DESENHA = new Set(['caneta', 'marcador'])

function Botao({ icone: Icone, rotulo, ativo, aviso, onClick, title, desabilitado }) {
  return (
    <button type="button" onClick={onClick} title={title ?? rotulo} disabled={desabilitado}
      aria-pressed={ativo === undefined ? undefined : ativo}
      className={`flex h-12 min-w-12 flex-col items-center justify-center gap-0.5 rounded-xl px-3
      ...
      <Icone size={20} />
      <span className="hidden sm:block">{rotulo}</span>
    </button>
  )
}

export default function BarraApresentacao({
  slideAtual, totalSlides, nomes, irPara,
  ferramenta, mudarFerramenta,
  desfazer, podeDesfazer, limparSlide, temTracosNoSlide,
  ...
  return (
    <>
        ...
        <div className="fixed inset-0 z-40 flex items-end justify-center bg-slate-950/60 p-4 backdrop-blur-sm print:hidden"
          ...
          <div className="mb-24 max-h-[70vh] w-full max-w-3xl overflow-y-auto rounded-2xl bg-slate-900 p-3 shadow-2xl"
            ...
            <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
              {nomes.map((n, i) => (
                <button key={n.slide + i} type="button"
                  onClick={() => { irPara(i); setIndice(false) }}
                  className={`flex min-h-12 items-center gap-3 rounded-xl px-3 py-2 text-left text-sm transition-colors
      ...
      <div className="fixed inset-x-0 bottom-0 z-40 flex flex-col items-center print:hidden"
        ...
        {/* tintas — só aparecem quando há o que pintar */}
        {DESENHA.has(ferramenta.tipo) && (
          <div className="mb-2 flex items-center gap-2 rounded-full bg-slate-900/92 px-3 py-2 shadow-xl backdrop-blur">
            {TINTAS.map((t) => (
              <button key={t.id} type="button" title={t.rotulo}
        ...
        <div className="flex w-full max-w-6xl flex-wrap items-center justify-center gap-1 bg-slate-900/92 px-2 py-2
            shadow-[0_-8px_30px_rgba(2,6,23,0.35)] backdrop-blur sm:mx-3 sm:mb-3 sm:w-auto sm:rounded-2xl">

          {/* navegação */}
          <Botao icone={ChevronLeft} rotulo="Voltar" onClick={() => irPara(slideAtual - 1)}
            desabilitado={slideAtual === 0} title="Slide anterior (←)" />
          <button type="button" onClick={() => setIndice(true)}
            ...
            <LayoutGrid size={18} />
            <span className="text-sm tabular">{slideAtual + 1}<span className="opacity-50">/{totalSlides}</span></span>
          </button>
          <Botao icone={ChevronRight} rotulo="Avançar" onClick={() => irPara(slideAtual + 1)}
            desabilitado={slideAtual >= totalSlides - 1} title="Próximo slide (→ ou espaço)" />

          <span className="mx-1 h-8 w-px bg-white/10" />

          {/* ferramentas */}
          {FERRAMENTAS.map((f) => (
            <Botao key={f.id} icone={f.icone} rotulo={f.rotulo} title={`${f.rotulo} — ${f.dica}`}
          ...
          <Botao icone={Undo2} rotulo="Desfazer" onClick={desfazer} desabilitado={!podeDesfazer}
            title="Desfazer o último traço deste slide" />
          ...
          <span className="mx-1 h-8 w-px bg-white/10" />
          <Botao icone={Minimize2} rotulo="Sair" onClick={sair} title="Sair da apresentação (Esc)" />
          <Botao icone={X} rotulo="Ocultar" onClick={() => setRecolhida(true)}
            title="Esconder os controles e deixar o slide limpo" />
        </div>
      </div>
    </>
  )
